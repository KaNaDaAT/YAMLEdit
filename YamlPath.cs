	...
	public static class YamlPath {
		...
		public static YamlNode NodeByKey(this YamlNode node, string key) {
			...
			if(sequence != null) {
				...
				bool isSequence = IsSequenceInception(key);
		...
		public static void Put(this YamlNode node, string key, object value) {
			...
			if(sequence != null) {
				if(valueNode == null) {
					...
					int i = sequence.Children.IndexOf(valueNode);
					sequence.Children[sequence.Children.IndexOf(valueNode)] = ConvertValueToYaml(value);
				}
			}
		}

		public static bool IsScalarNode(this YamlNode node) {
			return (node as YamlScalarNode) != null;
		}
